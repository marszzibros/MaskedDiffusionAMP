...
SAFE_TOKEN = re.compile(r'(\[[^\]]+\]|Br?|Cl?|N|O|S|P|F|I|b|c|n|o|s|p|\(|\)|\.|=|#|-|\+|\\|/|:|~|@|\?|>|\*|\$|%[0-9]{2}|[0-9])')
RING_LABEL = re.compile(r'%\d\d|\d')


def renumber_safe(safe_str):
    """Relabel ring/attachment numbers in order of first appearance.

    SAFE inherits its numbering from RDKit's canonical traversal of the whole
    molecule, so which number names a given bond is unpredictable from the
    string: position carries no information about the label (r = -0.03 over the
    corpus). Renumbering sequentially makes position predict the label
    (r = +0.22), turning "recall an arbitrary integer 200 tokens later" into
    "use the next free one".

    That matters here and not for the original SAFE models because those are
    autoregressive -- they can look back and copy the first occurrence. Masked
    flow matching fills positions in random order, so it may have to emit the
    second occurrence before the first exists.

    Numbers are reused once a bond closes, since ~1% of molecules have more
    than 90 simultaneous attachments. The molecule is unchanged: ring labels
    are arbitrary names, only the pairing carries meaning.
    """
    out, open_map, in_use = [], {}, set()
    for tok in SAFE_TOKEN.findall(safe_str):
        if not RING_LABEL.fullmatch(tok):
            out.append(tok)
            continue
        if tok in open_map:                       # closing an open bond
            num = open_map.pop(tok)
            in_use.discard(num)
        else:                                     # opening a new one
            num = next(i for i in range(1, 100) if i not in in_use)
            in_use.add(num)
            open_map[tok] = num
        out.append(str(num) if num < 10 else f'%{num:02d}')
    return ''.join(out)
...
class AMPSafeDataset(Dataset):
    ...
    def __init__(self, data_path="molecular_dataset/dataset/data/", max_length=None, mic_bins=10,
        ...
        safe_strings = unique['safe'].tolist()
        if canonical_labels:
            safe_strings = [renumber_safe(s) for s in safe_strings]
