...
# def score_sequence(seq_string):
#     """External Biopython scoring function."""
#     clean_seq = seq_string.replace('?', '').replace('[MASK]', '')
#     if len(clean_seq) == 0:
#          return -1
#     return clean_seq.count('R') + clean_seq.count('K')

# def generate_sample(self, tokens_dict, num_samples=10, max_length=130, 
#                     K_samples=10, guidance_start_ratio=0.5):
#     self.model.eval()
#     self.noiser.eval()
...
#         start_guidance_step = int(self.num_steps * (1.0 - guidance_start_ratio))
...
#             # =================================================================
#             # BRANCHING & SOFT TOP-3 FILTERING
#             # =================================================================
#             if i >= start_guidance_step:
#                 # 1. Expand tensors by K_samples
#                 q_xs_x_expanded = q_xs_x.repeat_interleave(K_samples, dim=0)
#                 x_expanded = x.repeat_interleave(K_samples, dim=0)
                
#                 # 2. Sample K times
#                 _x_expanded = self.diffusion._sample_categorical(q_xs_x_expanded)
                
#                 # 3. Apply mask logic
#                 copy_flag_x_expanded = (x_expanded != self.diffusion.mask_index).to(x_expanded.dtype)
#                 x_next_expanded = copy_flag_x_expanded * x_expanded + (1 - copy_flag_x_expanded) * _x_expanded
                
#                 # 4. Decode and score
#                 x_next_np = x_next_expanded.detach().cpu().numpy()
#                 lengths_expanded = lengths.repeat_interleave(K_samples, dim=0).detach().cpu().numpy()
                
#                 scores = []
#                 for seq_arr, length in zip(x_next_np, lengths_expanded):
#                     seq_string = ''.join(index_to_token.get(tok, '?') for tok in seq_arr[:length])
#                     scores.append(score_sequence(seq_string))
                
#                 # 5. Find the Top 3 and randomly select one
#                 scores_tensor = torch.tensor(scores, device=self.device).view(x.shape[0], K_samples)
                
#                 # Get the indices of the top 3 scores for each item in the batch
#                 # Ensure we don't try to pull top 3 if K_samples is less than 3
#                 top_k_val = min(3, K_samples) 
#                 _, top3_indices = scores_tensor.topk(top_k_val, dim=1) 
                
#                 # Randomly pick an index (0, 1, or 2) for each batch item
#                 random_choices = torch.randint(0, top_k_val, (x.shape[0],), device=self.device)
                
#                 # Extract the actual chosen K-index
#                 selected_k_indices = top3_indices[torch.arange(x.shape[0]), random_choices]
                
#                 # 6. Gather the selected x_next states
#                 batch_offsets = torch.arange(x.shape[0], device=self.device) * K_samples
#                 gather_indices = batch_offsets + selected_k_indices
                
#                 x_next = x_next_expanded[gather_indices]

#             else:
#                 # Standard single sampling
#                 _x = self.diffusion._sample_categorical(q_xs_x)
#                 copy_flag_x = (x != self.diffusion.mask_index).to(x.dtype)
#                 x_next = copy_flag_x * x + (1 - copy_flag_x) * _x
#             # =================================================================
...
#         t = timesteps[-1] * torch.ones(x.shape[0], 1, device=self.device)
#         sigma = self.noiser(t)[0].squeeze().to(self.device)
#         x = self.model(x, torch.zeros_like(sigma), lengths) 
#         x = x.argmax(dim=-1) 
...
#         sequences = []
#         for i, (seq, length) in enumerate(zip(x, lengths)):
#             sequence = ''.join(index_to_token.get(token, '?') for token in seq[:length])
#             sequences.append(sequence)
